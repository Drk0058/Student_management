...
@login_required()
def chat_room(request, stud_id, room_name):
    stud = get_object_or_404(Student, Rollno=stud_id).name
    return render(request, 'info/chatroom.html', {'stud_id': stud_id, 'stud_name': stud, 'room_name': room_name})
...
def change_att(request, att_id):
    ...
    a.save()
    return HttpResponseRedirect(reverse('t_attendance_detail', args=(a.student.Rollno, a.course_id)))
...
def timetable(request, class_id):
    ...
    matrix = [[['' for e in range(2)] for i in range(13)] for j in range(5)]
    meeting_link = [[[" "for e in range(2)] for i in range(13)] for j in range(5)]
    for i, d in enumerate(DAYS_OF_WEEK):
        ...
        for j in range(13):
            ...
            try:
                ...
                matrix[i][j][0] = a.assign.course_id + "\n" + '(' + a.assign.course.status + ')'+"\n" + a.assign.course.room
                matrix[i][j][1] = a.assign.teacher.meeting_link
            except AssignTime.DoesNotExist:
                pass
            t += 1

    context = {'matrix': matrix, 'link': meeting_link}
