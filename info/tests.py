...
class InfoTest(TestCase):
    ...
    def create_course(self, id='CS510', name='Data Struct', status='DS'):
        dept = self.create_dept(id='CS2')
        return Course.objects.create(id=id, dept=dept, name=name, status=status)
    ...
    def test_no_attendance(self):
        s = self.create_student()
        self.client.login(username='test_user', password='test_password')
        response = self.client.get(reverse('attendance', args=(s.Rollno,)))
    ...
    def test_no_attendance__detail(self):
        ...
        cr = self.create_course()
        self.client.login(username='test_user', password='test_password')
        resp = self.client.get(reverse('attendance_detail', args=(s.Rollno, cr.id)))
    ...
    def test_attendance__detail(self):
        ...
        resp = self.client.get(reverse('attendance_detail', args=(s.Rollno, cr.id)))
        self.assertEqual(resp.status_code, 200)
        self.assertQuerysetEqual(resp.context['att_list'], ['<Attendance: ' + s.name + ' : ' + cr.status + '>'])
